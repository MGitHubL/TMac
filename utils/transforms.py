...
def rgb_to_grayscale(vid, channel=0):
    ...
    return (
        0.2989 * vid.select(channel, 0)
        + 0.5870 * vid.select(channel, 1)
        + 0.1140 * vid.select(channel, 2)
    ).to(vid.dtype)
...
def adjust_saturation(vid, saturation_factor, channel=1, gray_channel=0):
    ...
    return _blend(
        vid,
        rgb_to_grayscale(vid, gray_channel).unsqueeze(gray_channel),
        saturation_factor,
    )
...
def random_adjust_brightness(vid, brightness_factor, consistent, channel=1):
    N = vid.size(channel)
    if consistent:
        brightness_factor = np.array(
            [random.uniform(brightness_factor[0], brightness_factor[1])] * N
        )
    else:
        brightness_factor = np.random.uniform(
            brightness_factor[0], brightness_factor[1], size=(N,)
        )
...
def random_adjust_contrast(vid, contrast_factor, consistent, channel=1, gray_channel=0):
    N = vid.size(channel)
    if consistent:
        contrast_factor = np.array(
            [random.uniform(contrast_factor[0], contrast_factor[1])] * N
        )
    else:
        contrast_factor = np.random.uniform(
            contrast_factor[0], contrast_factor[1], size=(N,)
        )
...
def random_adjust_saturation(
    vid, saturation_factor, consistent, channel=1, gray_channel=0
):
    N = vid.size(channel)
    if consistent:
        saturation_factor = np.array(
            [random.uniform(saturation_factor[0], saturation_factor[1])] * N
        )
    else:
        saturation_factor = np.random.uniform(
            saturation_factor[0], saturation_factor[1], size=(N,)
        )
...
class ColorJitter(object):
    ...
    @staticmethod
    def get_params(
        brightness, contrast, saturation, consistent, n_channel, gray_channel
    ):
        ...
        if brightness is not None:
            transforms.append(
                torchvision.transforms.Lambda(
                    lambda vid: random_adjust_brightness(
                        vid, brightness, consistent, n_channel
                    )
                )
            )
        ...
        if contrast is not None:
            transforms.append(
                torchvision.transforms.Lambda(
                    lambda vid: random_adjust_contrast(
                        vid, contrast, consistent, n_channel, gray_channel
                    )
                )
            )
        ...
        if saturation is not None:
            transforms.append(
                torchvision.transforms.Lambda(
                    lambda vid: random_adjust_saturation(
                        vid, saturation, consistent, n_channel, gray_channel
                    )
                )
            )
