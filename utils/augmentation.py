    ...
    def __call__(self, imgmap):
        if random.random() < self.threshold:  # do RandomRotation
            if self.consistent:
                ...
                return [
                    i.rotate(
                        np.random.randint(-self.degree, self.degree, 1)[0], expand=True
                    )
                    for i in imgmap
                ]
...
class GaussianBlur(object):
    ...
    def __init__(self, sigma=[0.1, 2.0], seq_len=0):
        self.sigma = sigma
        self.seq_len = seq_len
...
class TransformController:
    ...
    def __str__(self):
        string = "TransformController: %s with weights: %s" % (
            str(self.transform_list),
            str(self.weights),
        )
        return string
